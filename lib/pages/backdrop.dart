...
class BackDrop extends StatefulWidget {
  BackDrop({Key key, this.backLayer, this.backTitle, this.scrollBloc})
      : super(key: key);
...
class BackDropState extends State<BackDrop>
  ...
  void initState() {
    ...
    scrollStateSubscription =
        widget.scrollBloc.scrollStateOutput.listen(onScrollStateChange);

    initialHorizontalScrollProgressSubscription =
        widget.scrollBloc.horizontalScrollOutput.listen((value) {
      setState(() {
        t = value;
      });
    });
  ...
  Widget _buildStack(BuildContext context, BoxConstraints boxConstraints) {
    final Animation<RelativeRect> frontRelativeRect = new RelativeRectTween(
      begin: new RelativeRect.fromLTRB(0.0, _kBackAppBarHeight, 0.0, 0.0),
          ...
          0.0, boxConstraints.biggest.height, 0.0, 0.0),
    ).animate(_controller);

    final Animation<RelativeRect> backRelativeRect = new RelativeRectTween(
      begin:
          new RelativeRect.fromLTRB(0.0, _kBackAppBarHeight + 12.0, 0.0, 0.0),
...
class MyScrollPhysics extends ScrollPhysics {
  MyScrollPhysics(
      {ScrollPhysics parent, this.controller, DoubleCallback this.backdrop})
      : super(parent: parent);
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    ...
    if (direction == 1.0) {
      if (position.pixels < position.minScrollExtent ||
          position.pixels - offset < position.minScrollExtent) {
...
class CoolScrollSimulation extends Simulation {
  CoolScrollSimulation({
    ...
    if (position < leadingExtent) {
      ...
      if (velocity > 0.0 && finalX > trailingExtent) {
        ...
        _springSimulation = new StopSimulation(
            tolerance: tolerance, stopPosition: leadingExtent);
        topSpring = true;
        assert(_springTime.isFinite);
      } else {
  ...
  Simulation _simulation(double time) {
    ...
    if (time > _springTime) {
      if (topSpring && !controller.isAnimating) {
        controller.animateWith(new ScrollSpringSimulation(spring, 0.0, 0.0,
            -_frictionSimulation.dx(time) / (backdrop() * 1.25)));
      }
  ...
  bool isDone(double time) =>
      controller.isAnimating || _simulation(time).isDone(time - _timeOffset);
...
class StopSimulation extends Simulation {
  StopSimulation({Tolerance tolerance, this.stopPosition})
      : super(tolerance: tolerance);
...
class AnimatedBackground extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double localProgress =
        ((t - progressPerChild * currentIndex) / progressPerChild)
            .clamp(0.0, 1.0);
    ...
    Color secondBottomColor =
        colors[(currentIndex + 1) % colors.length].shade300;

    Color topColor = Color.lerp(firstTopColor, secondTopColor, localProgress);
    Color bottomColor =
        Color.lerp(firstBottomColor, secondBottomColor, localProgress);

    List<ShapeBackgroundPainterBuilder> painterBuilders = [
      (t, c) => [
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(0.5 * c.maxWidth, 0.08 * c.maxHeight,
                    c.maxWidth * 1.1, c.maxWidth * 1.1),
                end: new Rect.fromLTWH(
                    c.maxWidth, 0.0, c.maxWidth * 0.9, c.maxWidth * 0.9),
              ).lerp(t),
              child: new Material(
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(-0.1 * c.maxWidth, 0.75 * c.maxHeight,
                    c.maxWidth * 0.5, c.maxWidth * 0.5),
                end: new Rect.fromLTWH(-0.5 * c.maxWidth, 0.9 * c.maxHeight,
                    c.maxWidth * 0.5, c.maxWidth * 0.5),
              ).lerp(t),
              child: new Material(
                type: MaterialType.circle,
      ...
      (t, c) => [
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.1, c.maxHeight * 0.1,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth * 0.25, 0.0, c.maxWidth * 0.25,
                    c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
                ...
                child: Transform.rotate(
                  ...
                  child: new CustomPaint(
                    painter:
                        new PathPainter(trianglePath, painter: trianglePaint),
                  ),
                ),
              ),
            ),
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.5, c.maxHeight * 0.01,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth * 0.5, -c.maxWidth * 0.25,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.75, c.maxHeight * 0.21,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight * 0.2,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.9, c.maxHeight * 0.9,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight, c.maxWidth * 0.25,
                    c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(-c.maxWidth * 0.05, c.maxHeight * 0.85,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth * 0.25, c.maxHeight,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
      ...
      (t, c) => [
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.5, c.maxHeight * 0.4,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight * 0.6,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.5, c.maxHeight * 0.5,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight * 0.6,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.5, c.maxHeight * 0.6,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight * 0.6,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.2, c.maxHeight * 0.8,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth, c.maxHeight * 0.8,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
                scale: Tween<double>(begin: 4.0, end: 1.0).lerp(t),
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.2, c.maxHeight * 0.9,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth, c.maxHeight * 0.8,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.2, c.maxHeight,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth, c.maxHeight * 0.8,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
      ...
      (t, c) => [
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(c.maxWidth * 0.7, c.maxHeight * 0.2,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(c.maxWidth, c.maxHeight * 0.3,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
                scale: Tween<double>(begin: 4.0, end: 1.0).lerp(t),
            ...
            Positioned.fromRect(
              rect: new RectTween(
                begin: new Rect.fromLTWH(-c.maxWidth * 0.1, c.maxHeight * 0.7,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
                end: Rect.fromLTWH(-c.maxWidth * 0.4, c.maxHeight * 0.6,
                    c.maxWidth * 0.25, c.maxWidth * 0.25),
              ).lerp(t),
              child: Transform.scale(
                scale: Tween<double>(begin: 3.0, end: 1.0).lerp(t),
        ...
        builder: (context, constraints) {
          ...
          for (int i = 0; i < painterBuilders.length; i++) {
            children.addAll(painterBuilders[i](
                progressForIndex(i, numItems, painterBuilders.length)
                    .clamp(0.00001, 1.0),
                constraints));
          }
  ...
  double progressForIndex(int builderIndex, int numItems, int numBuilders) {
    double normalizedProgress =
        ((t * (numItems - 1) - builderIndex) % numBuilders) / numBuilders;
    int index = builderIndex +
        (normalizedProgress * 2).floor() * numBuilders +
        ((t * (numItems - 1) - builderIndex) / numBuilders).floor() *
            numBuilders;

    double progress =
        ((((t * 2) - progressPerChild * index) / progressPerChild) - index)
            .abs();
...
typedef List<Widget> ShapeBackgroundPainterBuilder(
    double t, BoxConstraints constraints);
...
class BackgroundPainter extends ShapeBackgroundPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double circle1Radius =
        new Tween<double>(begin: size.width * 0.4, end: size.width * 0.2)
            .lerp(t);
    Rect circle1Rect =
        Rect.fromCircle(center: circle1Position, radius: circle1Radius);
    Paint circle1Line = new Paint()
      ..shader = gradient.createShader(circle1Rect);
    ...
    double circle2Radius =
        new Tween<double>(begin: size.width * 0.2, end: size.width * 0.1)
            .lerp(t);
    Rect circle2Rect =
        Rect.fromCircle(center: circle2Position, radius: circle2Radius);
    Paint circle2Line = new Paint()
      ..shader = gradient.createShader(circle2Rect);

    canvas.drawShadow(new Path()..addOval(circle1Rect),
        Colors.black.withOpacity(0.5), 10.0, true);
    canvas.drawCircle(circle1Position, circle1Radius, circle1Line);

    canvas.drawShadow(new Path()..addOval(circle2Rect),
        Colors.black.withOpacity(0.5), 10.0, true);
